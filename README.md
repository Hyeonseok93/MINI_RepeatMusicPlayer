...
PySide6 기반의 **음악 트랙별 개별 반복 재생 & 통합 병합 유틸리티**입니다. 여러 음원 파일(.mp3, .wav, .flac, .ogg, .m4a 등)을 불러와 **각 곡마다 원하는 반복 횟수(0회~999회)를 자유롭게 지정**하여 순차적으로 정밀 반복 재생할 수 있으며, 설정된 반복 횟수와 순서가 정확히 반영된 **단일 오디오 파일로 일괄 병합(Concat)** 추출하는 편리한 프리셋 기능을 제공합니다.
...
- **🏷️ 스마트 말줄임표 UI**: 긴 제목/경로의 자동 `...` 처리 및 접미사(`• 반복 k/n회`) 노출 보장
