...
여러 음원 파일(.mp3, .wav, .flac, .ogg, .m4a 등)을 불러와 **각 곡마다 원하는 반복 횟수(0회~999회)를 자유롭게 지정**하여 순차적으로 정밀 반복 재생할 수 있습니다.  
또한 설정된 반복 횟수와 순서가 정확히 반영된 **단일 오디오 파일로 일괄 병합(Concat)** 추출하는 편리한 프리셋 기능을 제공합니다.
...
- **🔁 개별 트랙 반복 횟수 설정 (Custom Track Repeats)**: 각 음원마다 0회부터 999회까지 자유롭게 반복 횟수를 지정하여 재생 대기열을 구성합니다.
- **🎯 원클릭 일괄 프리셋 & 셔플**: `전체 1회`, `전체 0회`, `+1회 증가`, `-1회 차감` 버튼 및 `순서 셔플` 무작위 믹스 기능을 지원합니다.
- **🔀 트랙 순서 재정렬 및 순정 탐색기**: `▲` / `▼` 순서 이동 버튼을 통한 안전한 트랙 정렬 및 익숙한 Windows 순정 파일/폴더 선택 창을 제공합니다.
- **📎 FFmpeg 오디오 일괄 병합 (Merge/Concat)**: 트랙 순서 및 반복 횟수가 100% 반영된 단일 음원 파일(.mp3 등)로 인코딩하여 추출합니다.
- **🏷️ 스마트 말줄임표 & 반복 정보 노출**: 긴 트랙명/폴더 경로 자동 말줄임표(`...`) 처리 및 하단 상태바의 ` • 반복 (k/n회)` 접미사 100% 가독성을 보장합니다.
- **⌨️ 직관적인 단축키**: `Space` (재생/일시정지), `←` / `→` (5초 이동), `↑` / `↓` (볼륨 조절), `F1` (도움말) 지원.
